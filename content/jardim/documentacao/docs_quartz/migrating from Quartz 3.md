...
4. **A new extensible [[content/redearte/jardim/documentacao/docs_quartz/configuration]] and [[content/redearte/jardim/documentacao/docs_quartz/configuration#Plugins|plugin]] system**: Quartz 3 was hard to configure without technical knowledge of how Hugo's partials worked. Extensions were even hard to make. Quartz 4's configuration and plugin system is designed to be extended by users while making updating to new versions of Quartz easy.

## Things to update

- You will need to update your deploy scripts. See the [[content/redearte/jardim/documentacao/docs_quartz/hosting]] guide for more details.
- Ensure that your default branch on GitHub is updated from `hugo` to `v4`.
- [[content/redearte/jardim/documentacao/docs_quartz/features/folder and tag listings|Folder and tag listings]] have also changed.
...
- If you customized the layout of Quartz 3, you may need to translate these changes from Go templates back to JSX as Quartz 4 no longer uses Hugo. For components, check out the guide on [[content/redearte/jardim/documentacao/docs_quartz/advanced/creating components]] for more details on this.
