...
Because the Markdown generated by ox-hugo is not pure Markdown but Hugo specific, we need to transform it to fit into Quartz. This is done by the [[docs/plugins/OxHugoFlavoredMarkdown]] plugin. Even though this plugin was written with `ox-hugo` in mind, it should work for any Hugo specific Markdown.
...
This functionality is provided by the [[docs/plugins/OxHugoFlavoredMarkdown]] plugin. See the plugin page for customization options.
